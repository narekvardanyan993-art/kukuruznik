/* ВНИМАНИЕ: этот файл СОБРАН автоматически из src/engine/.
...
(function (global) {
  ...
  var BUILD = '60';     // видно на самой странице — чтобы не гадать, свежая ли версия
  ...
  function Engine(sceneCanvas, paperCanvas, model) {
    ...
    for (var ci = 0; ci < 7; ci++) {
      /* Раньше облако жило в экранных координатах и просто ползло
         слева направо по кадру — при повороте камеры оставалось на
         месте, будто приклеено к стеклу, и на некоторых углах даже
         перекрывало башню снизу, хотя должно было быть далеко в
         небе. Теперь у него, как у звёзд, есть азимут В МИРЕ — угол
         на который поворачивать нужно вместе с камерой — а не сдвиг
         по экрану. */
      this.clouds.push({
        th: rc2() * 6.28318, y: 0.04 + rc2() * 0.26,
        s: 0.045 + rc2() * 0.045,
        v: 0.006 + rc2() * 0.014,         // радиан в секунду — медленный снос по миру
        ph: rc2() * 6.28
      });
    }
    var rb = seeded(404);
  ...
  Engine.prototype.resize = function () {
    ...
    var iw = global.innerWidth, ih = global.innerHeight;
    var scr = global.screen || {};

    /* Холодный запуск отдельным приложением («На экран Домой»): на
       части айфонов innerWidth/innerHeight на первых кадрах приходят
       нулевыми, и раньше мы просто ждали следующего кадра (см. boot()).
       Но на практике это не всегда решалось за полторы секунды — экран
       так и оставался пустым. Запасной вариант надёжнее ожидания:
  ...
  Engine.prototype.drawArarat = function () {
    ...
    /* Угловой размер. По-честному 4385 м на 60 км — это 0.073 радиана.
       Уже было накручено до 0.150 -- глаз с земли всё равно просил
       ещё: гора должна доминировать над кадром, а не быть одной из
       деталей. Подняли до 0.168. При 0.175 массив выходил шире экрана
       и Малый Арарат срезался -- выше этого не поднимать. */
    var H = 0.168 * F / Math.cos(tt);                // высота над горизонтом
    ...
    /* Большой и Малый Арарат должны стоять зеркально тому, что тут
       было нарисовано: из Еревана Большой (со снегом) справа, Малый —
       слева (проверено: подписи к фото вида из Еревана и координаты
       вершин в Википедии). Весь силуэт посчитан как один узор вокруг
       gx, поэтому переворачиваем координаты по горизонтали одним
       transform, а не переписываем числа в трёх фигурах отдельно. */
    ctx.save();
    ctx.translate(gx, 0);
    ctx.scale(-1, 1);
    ctx.translate(-gx, 0);
